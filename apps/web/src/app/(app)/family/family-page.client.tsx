...
import { usePreloadedFamilyGraph } from "./family-graph-context";
...
type GraphData = FunctionReturnType<typeof api.family.getGraph> | undefined;

function FamilyGraphScene({ graphData }: { graphData: GraphData }) {
...
function FamilyPageContent() {
  const preloaded = usePreloadedFamilyGraph();
  if (preloaded) return <PreloadedFamilyPage preloaded={preloaded} />;
  return <LiveFamilyPage />;
}

function PreloadedFamilyPage({
  preloaded,
}: {
    ...
    <div className="flex h-full min-h-0 w-full flex-col">
      ...
      <div className="min-h-0 flex-1">
        {activeTab === "our-family" ? (
          view === "tree" ? (
            <ReactFlowProvider>
              <FamilyGraphScene graphData={graphData} />
            </ReactFlowProvider>
          ) : (
